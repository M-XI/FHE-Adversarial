...
def get_dataloader(
    ...
    max_examples = param.get("dataset_size", None) if debug is None else debug
...
def torch_inference(
    ...
    for x, y in tqdm(data, disable=verbose is False):
        ...
        predictions.extend(yhat.argmax(1).tolist())
...
def fhe_simulation_inference(
    ...
    for x, y in tqdm(data_loader, disable=disable_tqdm):
        if preprocessor is not None:
            x = preprocessor(x, y)
        ...
        pred = quantized_module.forward(x, fhe=mode)
